...
bool GameObject::IsInCamera()
{
	...
	if (postX + collider->left >= Sprite::cameraX + 512)
		return false;
	if (postY + collider->top <= Sprite::cameraY - 416)
		return false;
	if (postY + collider->bottom >= Sprite::cameraY-96)
		return false;
	return true;
}
...
void GameObject::SlideFromGround(GameObject *target, const float &_DeltaTime, const float &_CollisionTimeScale)
{
	...
	else if (normaly < -0.1f)	// tông ở dưới
	{
		//this->postY = (target->postY - target->collider->top + this->collider->bottom) - 0.1f;
		this->velocityY = 0.0f;
	...
	// BUG NHẢY LÊN GẠCH
	/*
	else if (normaly < -0.1f)	// tông ở dưới để nhảy lên trên
	{
		this->postY = (target->postY + target->collider->top - this->collider->bottom) + 0.1f;
		//velocityY = 0.0f;
	}
