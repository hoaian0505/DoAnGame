...
Clock::Clock(LPD3DXSPRITE _SpriteHandler, World *_manager)
{
	manager = _manager;
	objectType = GETCLOCK;
	...
	texture = new Texture(_SpriteHandler, "Resources\\Sprites\\Items\\5.png");
...
void Clock::Update(const float &_DeltaTime)
{
	if (!isActive)
		return;
...
void Clock::RenderCamera()
{
	if (manager->Simon->subWeapon == CLOCK)
...
void Clock::CollisionObject(float _DeltaTime)
{
